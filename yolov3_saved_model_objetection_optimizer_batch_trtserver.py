...
parser.add_argument("--output_trt_dir", default="model/yolov3/trt", help="export model directory", type=str)
...
def export() -> object:
    ...
    with tf.Session() as sess:
        ...
        sbbox_tensor = tf.reshape(tf.convert_to_tensor(sbbox_tensor), shape=[batch_size, -1, 5 + class_num])
        mbbox_tensor = tf.reshape(tf.convert_to_tensor(mbbox_tensor), shape=[batch_size, -1, 5 + class_num])
        lbbox_tensor = tf.reshape(tf.convert_to_tensor(lbbox_tensor), shape=[batch_size, -1, 5 + class_num])
        output_tensor = tf.concat([sbbox_tensor, mbbox_tensor, lbbox_tensor], 1)
        ...
        scores_tensor_info = tf.saved_model.utils.build_tensor_info(tensor_result[:, :, 5])
        ...
        converter = trt.TrtGraphConverter(input_saved_model_dir="/home/zyc/tensorflow-yolov3-master/model/yolov3/1",
                                          max_batch_size=16,
                                          max_workspace_size_bytes=4000000000,
                                          is_dynamic_op=True,
                                          precision_mode="FP16")
...
def preprocess_image(image_buffer):
  ...
  # image = tf.io.decode_base64(image_buffer)
  # tf.image.
  # with tf.control_dependencies([tf.print(image_buffer)]):
